using System;
using System.Collections.Generic;
using System.Reflection;
using UnityEngine;

namespace FrameWork;

public class Actor
{
	private int Index = -999999;

	private GameObject _gameObject;

	public Transform transform => _gameObject.transform;

	public Actor()
	{
		ActorInfoAttribute customAttribute = GetType().GetCustomAttribute<ActorInfoAttribute>();
		if (customAttribute != null && !(customAttribute.PrefabName == ""))
		{
			GameObject gameObject = null;
			gameObject = ABMrg.Load<GameObject>(customAttribute.PrefabName, isAutoRelease: true, isAdd: false);
			_gameObject = UnityEngine.Object.Instantiate(gameObject);
			_gameObject.SetActive(value: false);
			_gameObject.AddComponent<ActorMono>().SetActor(this);
			_gameObject.SetActive(value: true);
		}
	}

	public Actor(Transform trans)
	{
		Type type = GetType();
		ActorInfoAttribute customAttribute = type.GetCustomAttribute<ActorInfoAttribute>();
		bool isAdd = true;
		UiModeAttribute customAttribute2 = type.GetCustomAttribute<UiModeAttribute>();
		if (customAttribute2 != null)
		{
			isAdd = customAttribute2.isCanRemove;
		}
		if (customAttribute != null && !(customAttribute.PrefabName == ""))
		{
			GameObject gameObject = null;
			gameObject = ABMrg.Load<GameObject>(customAttribute.PrefabName, isAutoRelease: true, isAdd);
			_gameObject = UnityEngine.Object.Instantiate(gameObject, trans);
			_gameObject.SetActive(value: false);
			_gameObject.AddComponent<ActorMono>().SetActor(this);
			_gameObject.SetActive(value: true);
		}
	}

	public virtual bool IsCanJoinPause()
	{
		return true;
	}

	public GameObject GetGameObject()
	{
		return _gameObject;
	}

	public T AddComponent<T>() where T : Component
	{
		return _gameObject.AddComponent<T>();
	}

	public int GetIndex()
	{
		return Index;
	}

	public void SetIndex(int index)
	{
		Index = index;
	}

	public virtual void Awake()
	{
	}

	public virtual void Start()
	{
	}

	public virtual void OnEnable()
	{
	}

	public virtual void OnDisable()
	{
	}

	public virtual void Update(float deltaTime)
	{
	}

	public virtual void FixedUpdate(float deltaTime)
	{
	}

	public virtual void LateUpdate()
	{
	}

	public virtual void OnDestroy()
	{
	}

	public virtual void OnTriggerEnter()
	{
	}

	public virtual void OnTriggerEnter2D()
	{
	}

	public virtual void OnTriggerExit2D()
	{
	}

	public virtual void OnCollisionEnter()
	{
	}

	protected void AddListener(int eventType, int id, Action<List<object>> evt)
	{
		EventManager.AddListener(eventType, id, evt);
	}

	protected void RemoveListener(int eventType, int id, Action<List<object>> evt)
	{
		EventManager.RemoveListener(eventType, id, evt);
	}

	protected void DispatchEvent(int evtType, int evt, List<object> data = null)
	{
		EventManager.DispatchEvent(evtType, evt, data);
	}

	protected void AddListener(Enum eventType, Enum id, Action<List<object>> evt)
	{
		EventManager.AddListener((int)(object)eventType, (int)(object)id, evt);
	}

	protected void RemoveListener(Enum eventType, Enum id, Action<List<object>> evt)
	{
		EventManager.RemoveListener((int)(object)eventType, (int)(object)id, evt);
	}

	protected void DispatchEvent(Enum evtType, Enum evt, List<object> data = null)
	{
		EventManager.DispatchEvent((int)(object)evtType, (int)(object)evt, data);
	}
}
