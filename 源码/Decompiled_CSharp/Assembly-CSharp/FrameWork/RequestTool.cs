...
public class RequestTool
{
	...
	public void Send()
	{
		SingletonAsMono<Mono>.Instance.StartCoroutine(Request());
	}
	...
	public void Send(Action<Sprite> data, Action<string> err = null)
	{
		...
		IEnumerator GetTexture()
		{
			...
			if (www.isDone)
			{
				...
				data?.Invoke(Sprite.Create(texture, new Rect(0f, 0f, texture.width, texture.height), Vector2.zero, 48f, 0u, SpriteMeshType.FullRect));
			}
		}
	}
	...
	public async Task<Sprite> SendTaskAsTexture()
	{
		...
		IEnumerator GetTexture()
		{
			...
			if (www.isDone)
			{
				Progress?.Invoke(1f, size);
				Texture2D texture = ((DownloadHandlerTexture)www.downloadHandler).texture;
				tcs.SetResult(Sprite.Create(texture, new Rect(0f, 0f, texture.width, texture.height), Vector2.zero, 48f, 0u, SpriteMeshType.FullRect));
			}
		}
	}
