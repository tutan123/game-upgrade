...
public static class Easing
{
	...
	public static Func<float, float> GetFunction(Preset preset)
	{
		...
		case Preset.InOutExpo:
			...
			break;
		case Preset.Random:
			result = GetFunction((Preset)UnityEngine.Random.Range(0, 17));
			break;
		case Preset.RandomNotStep:
			result = GetFunction((Preset)UnityEngine.Random.Range(1, 17));
			break;
		}
		return result;
	}
	...
	public static float PowerEaseInOut(float t, float power)
	{
		if (t < 0.5f)
		{
			return PowerEaseIn(t * 2f, power) / 2f;
		}
		return PowerEaseOut(t * 2f - 1f, power) / 2f + 0.5f;
	}
	...
	public static float OutExpo(float t)
	{
		...
		if (t != 1f)
		{
			result = 0f - Mathf.Pow(2f, -10f * t) + 1f;
		}
		return result;
	}
	...
	public static float InOutExpo(float t)
	{
		...
		if (t > 0f)
		{
			...
			if (t < 1f)
			{
				...
				{
					t -= 1f;
					result = 0.5f * (0f - Mathf.Pow(2f, -10f * t) + 2f);
				}
			}
		}
		return result;
